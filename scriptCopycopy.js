...
getSessionId().then(cookie => {
  ...
  let session = cookie.split(' ')[0].slice(10, -1);   //TEXT=USUARIO
  instance.get(`https://steamcommunity.com/search/SearchCommunityAjax?text=Logisis&filter=users&sessionid=${session}&steamid_user=false&page=1`).then(res => {
